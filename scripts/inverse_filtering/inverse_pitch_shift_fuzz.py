...
ratio = 1/semitone2ratio(shift)

fft_size = (dat['spectrogram'].shape[0]-1)*2

start_time = time.time()
dat_shift2,_ = vocoder.encode(fs, x_shift, f0_method='swipe', is_requiem=True) # use requiem analysis and synthesis
...
for i in range(dat_shift['spectrogram'].shape[1]):
    interp_f = interpolate.interp1d(np.arange(0,fft_size//2+1),dat_shift['spectrogram'][:,i],kind='cubic',fill_value="extrapolate")
    if i == 300:
        plt.subplot(3,1,1)
        plt.plot(dat_shift['spectrogram'][:,i])  
        plt.subplot(3,1,2)
        plt.plot(dat_shift2['spectrogram'][:,i])  
        plt.subplot(3,1,3)
        plt.plot(interp_f(np.arange(0,fft_size//2+1)*ratio))
        plt.savefig("test.png")

    dat_shift['spectrogram'][:,i] = interp_f(np.arange(0,fft_size//2+1)*ratio)
print(dat_shift['spectrogram'][:20,300])
print(dat_shift2['spectrogram'][:20,300])
